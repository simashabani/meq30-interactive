...
export async function GET(request: Request) {
  ...
  const safeRedirect = redirect && redirect.startsWith("/") ? redirect : "/fa/journal";
  ...
  if (!code) {
    const failUrl = new URL("/fa/auth/status", url.origin);
  ...
  const statusUrl = new URL("/fa/auth/status", url.origin);
